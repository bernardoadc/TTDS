...
function registerPlugin (name, pluginApi, userOptions, allOptions) {
  pluginApi.initialize(lib, userOptions, allOptions) // allOptions is optional for plugins that use cross-plugin options
...
async function loadCorePlugins () {
  for (const pluginName of corePlugins) {
    ...
    registerPlugin(pluginName, pluginApi.default, lib.config[pluginName], lib.config)
  }
}
...
async function loadUserPlugins () {
  for (const plugin in lib.config) {
    if (!corePlugins.includes(plugin)) {
      ...
      const pluginApi = await getPackage(pluginPath)
      registerPlugin(plugin, pluginApi.default, lib.config[plugin], lib.config)
    }
  }
}
